...
function zoomToRect(svg: SVGSVGElement, rect: SVGRectElement) {
  ...
  // Adatta il viewBox all'aspect ratio del container
  if (vbW / vbH < containerAspect) {
...
function ObjectOverlay({
  ...
  const updateURL = async (oggettoCorrente: string, oggettoAltro: string) => {
    try {
      // Centra sulla stanza dell'oggetto corrente, non quella dell'URL
      const stanzaOggetto = await fetchStanza(oggettoCorrente);

      history.pushState(
        null,
        "",
        `/?stanza=${stanzaOggetto}/path/${oggettoCorrente}/${oggettoAltro}`
      );

      onClose();
      // Triggera il re-render del navigation (ricentra SVG sulla nuova stanza)
      window.dispatchEvent(new PopStateEvent("popstate"));
    } catch (err) {
